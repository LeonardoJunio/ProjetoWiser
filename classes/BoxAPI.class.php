...
class Box_API
{
    ...
    public function __construct($client_id = '', $client_secret = '', $box_subject_id = '')
    {
    ...
    public function get_token($json = false)
    {
        $url = $this->token_url;
        if (!empty($this->refresh_token)) {
            ...
            $params = array('grant_type' => 'client_credentials', 'client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'box_subject_type' => $this->box_subject_type, 'box_subject_id' => $this->box_subject_id, );
        }
        if ($json) {
            return $this->post($url, $params);
        } else {
            $response = json_decode($this->post($url, $params), true);
    ...
    public function get_all_file_version($file, $json = false)
    {
        $url = $this->build_url("/files/$file/versions");
        if ($json) {
            return $this->get($url);
        } else {
            return json_decode($this->get($url), true);
    ...
    public function delete_file_version($file, $fileVersion)
    {
        $url = $this->build_url("/files/$file/versions/$fileVersion");
        $return = json_decode($this->delete($url), true);
        if (empty($return)) {
            return 'The file has been deleted.';
        } else {
    ...
    public function promote_file_version($file, $fileVersion)
    {
        $url = $this->build_url("/files/$file/versions/current");
        $params = array('type' => 'file_version', 'id' => $fileVersion);
        return json_decode($this->post($url, json_encode($params)), true);
    }

    public function zip_folder_download($idPasta, $nomePasta)
    {
        $url = $this->build_url("/zip_downloads");
        $params = array('download_file_name' => $nomePasta, 'items' => [array('type' => 'folder', 'id' => $idPasta)]);
        return json_decode($this->post($url, json_encode($params)), true);
    }

    public function restore_file($fileVersion) //nao funciona aparentemente
    {
        $url = $this->build_url("/files/$fileVersion");
        $params = array();
        return json_decode($this->post($url, json_encode($params)), true);
    }

    public function get_trashed_file($file, $json = false) //nao funciona aparentemente
    {
        $url = $this->build_url("/files/$file/trash");
        if ($json) {
            return $this->get($url);
        } else {
            return json_decode($this->get($url), true);
